...
还有axios ajax jsonp 网络安全要复习了
gulp和webpack也是 不要忘记自己的无法替代性
不存在什么仁义道德 现在学会了什么是口头答应 什么是真正操作 太年轻了啊
9-3
子路由嵌套  vuex先配置好
子组件我觉得应该放到主文件的下面吧 还是说有什么奇怪的问题？
静态路由跳转 router-link
动态子路由跳转的配置
两者数据的传递
我就说怎么没讲用官网的例子来传 原来是用vuex
是否用vuex是看这个数据支持多少个组件或者说页面共同使用还是单独使用
阻碍
排行榜点进去的数据格式发生变化 提交方式是post 看看是否能突破
所有数据参数的作用现在开始要重新书写一次
最好是自己安排个开发文档来记载里面的数据参数
jsonp跨域请求数据
什么参数关联着什么数据 这个数据里又关联什么 轮流用jsonp去取去调用最终拿到自己想要的数据即可
是因为公钥不对 所以提交不对吗
3/26
这里有点忙最近忘了
新开的坑是websocket和echarts
echarts完成后就是elementUI了 还有树的算法